...
const LabLayout = ({ children, title }) => {
  ...
  const SidebarContent = () => (
    <div className="h-full flex flex-col bg-transparent text-slate-800">
      <div className="h-20 flex items-center px-6 border-b border-white/40">
        <div className="w-8 h-8 rounded-lg bg-gradient-to-tr from-purple-600 to-fuchsia-400 flex items-center justify-center mr-3 shadow-md shadow-purple-500/20">
          <Activity className="text-white w-5 h-5" />
        </div>
        <span className="text-xl font-extrabold tracking-tight text-slate-800">NovaCare <span className="text-xs bg-purple-600 text-white px-1.5 py-0.5 rounded font-mono ml-1">Lab</span></span>
      </div>
          ...
          <NavLink
            ...
            className={({ isActive }) => 
              `flex items-center px-4 py-3 rounded-xl font-medium transition-all duration-200 ${
                isActive 
                  ? 'bg-purple-500/20 text-purple-800 font-bold backdrop-blur-md shadow-sm' 
                  : 'text-slate-700 hover:bg-white/40 hover:text-purple-900'
              }`
            }
          >
      ...
      <div className="p-4 border-t border-white/40 bg-white/10 backdrop-blur-md">
        <div className="flex items-center mb-4 px-2">
          <ProfileAvatar user={user} className="w-10 h-10 mr-3" fallbackClassName="bg-purple-100 text-purple-800" />
  ...
  return (
    <div 
      ...
      style={{ backgroundImage: `url('/images/bg/lab-bg.jpg')` }}
    >
      {/* Light Overlay */}
        ...
        <div className="flex-1 md:ml-64 flex flex-col min-h-screen min-w-0">
          <header className="h-20 bg-white/40 backdrop-blur-xl border-b border-white/40 sticky top-0 z-20 flex items-center justify-between px-6 lg:px-10 shadow-sm">
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-2 bg-purple-500/10 backdrop-blur-md px-3.5 py-1.5 rounded-full border border-purple-500/20 shadow-sm hidden sm:flex">
